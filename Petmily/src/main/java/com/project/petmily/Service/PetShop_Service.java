...
public class PetShop_Service {
   ...
   public ModelAndView Shop_Input(Images_DTO idto, PetShop_DTO sdto, HttpServletResponse response) throws IOException {
      ...
      if (result > 0) {
          ...
          */
         mav.setViewName("redirect:/Shop_List");
      } else {
   ...
   public ModelAndView Item_Delivery(Delivery_DTO ddto) {
      ...
      int totalPrice = ddto.getItem_price() + 2000;
   ...
   /* 용품 검색 (연관검색어) */
      public List<PetShop_DTO> Item_Keword(String keyword) {
         
        System.out.println("서비스 keyword 값 : " + keyword);
         
        List<PetShop_DTO> searchResult = sdao.Item_Keyword(keyword);
         
      
         for(int i = 0; i < searchResult.size(); i++) {
            System.out.println("검색하여 조회한값 : " + searchResult.get(i));
         }
      ...
      /* 연관검색어 키워드를 통한 검색 */
      public ModelAndView Item_Search(String keyword) {
         mav = new ModelAndView();
         
         List<PetShop_DTO> Sellect = sdao.Item_Search(keyword);
         
         mav.addObject("ItemList", Sellect);
         mav.setViewName("PetShop_List");

...
public class PetShop_Controller {
	...
	/* 용품 검색 (연관검색어)*/
	   ...
	   public @ResponseBody List<PetShop_DTO> Item_Keword(@RequestParam(value="keyword", required=false)  String keyword){
	      
	      System.out.println("keyword값 : " + keyword);
	      
	      /* List<String> searchResult = ssvc.Item_Keword(keyword); */
	      
	      List<PetShop_DTO> searchResult = ssvc.Item_Keword(keyword);
	   ...
	   public ModelAndView Item_Search(@RequestParam("keyword") String keyword) {
	      
	      
	      
	      mav = ssvc.Item_Search(keyword);
